...
if(1==1) // Redirected from reviewOrder.php
{
	$itemsOutOfStock = [];
	...
	if ($itemsOutOfStock != [])
	{
		echo "<script>alert('Sorry, these items are out of stock: $itemsOutOfStock');
		</script>";
		//header ("Location: shoppingCart.php");
		//exit;
	}
	...
	if ($_POST["paymentMethod"] == "paypal") //Done for easy edits if website has other payment methods in future
	{
		...
		$padata = '&CURRENCYCODE='.urlencode($PayPalCurrencyCode).
				...
				'&PAYMENTREQUEST_0_ITEMAMT='.urlencode($_SESSION["DiscountSubTotal"]). // note: this amount will be compared with item amounts
